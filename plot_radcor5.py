...
def main():
    ...
    for theta_value in theta_values:
        ...
        inUnpol2FileName = "Radiated Data/Model_IQE_Smearing_g1-F1/radiated_model_" + thetaStr + "deg" + unpolLabel + ".dat"
        inLong2FileName = "Radiated Data/Model_IQE_Smearing_g1-F1/radiated_model_" + thetaStr + "deg" + longLabel + ".dat"
        inTrans2FileName = "Radiated Data/Model_IQE_Smearing_g1-F1/radiated_model_" + thetaStr + "deg" + transLabel + ".dat"
        inUnpol3FileName = "Radiated Data/Model_IQE_Smearing_g1+F1/radiated_model_" + thetaStr + "deg" + unpolLabel + ".dat"
        inLong3FileName = "Radiated Data/Model_IQE_Smearing_g1+F1/radiated_model_" + thetaStr + "deg" + longLabel + ".dat"
        inTrans3FileName = "Radiated Data/Model_IQE_Smearing_g1+F1/radiated_model_" + thetaStr + "deg" + transLabel + ".dat"
        ...
        try:
            with open(inUnpol3FileName, 'r') as infile:
                next(infile)
                dataUnpol3 = []
                for line in infile:
                    line.replace("\n","")
                    ...
                    for i in range(len(vals)):
                        vals[i] = float(vals[i])    # Ebeam, nu, xs_rad, xs_born, stat, syst
                    if (vals[0] != 10380 or vals[1] > (Ebeam-1)*1000): continue
                    dataUnpol3.append(vals)
        except IOError:
            print("Unable to open file:", inUnpol3FileName)
            return

        try:
            with open(inLong3FileName, 'r') as infile:
                next(infile)
                dataLong3 = []
                for line in infile:
                    line.replace("\n","")
                    ...
                    for i in range(len(vals)):
                        vals[i] = float(vals[i])    # Ebeam, nu, xs_rad, xs_born, stat, syst
                    if (vals[0] != 10380 or vals[1] > (Ebeam-1)*1000): continue
                    dataLong3.append(vals)
        except IOError:
            print("Unable to open file:", inLong3FileName)
            return

        try:
            with open(inTrans3FileName, 'r') as infile:
                next(infile)
                dataTrans3 = []
                for line in infile:
                    line.replace("\n","")
                    ...
                    for i in range(len(vals)):
                        vals[i] = float(vals[i])    # Ebeam, nu, xs_rad, xs_born, stat, syst
                    if (vals[0] != 10380 or vals[1] > (Ebeam-1)*1000): continue
                    dataTrans3.append(vals)
        except IOError:
            print("Unable to open file:", inTrans3FileName)
            return
        ...
        for d in dataUnpol3:
            xsradUnpol3[0].append(Ebeam - d[1]/1000)
            xsradUnpol3[1].append(d[2])
            xsbornUnpol3[0].append(Ebeam - d[1]/1000)
            xsbornUnpol3[1].append(d[3])
        for d in dataLong3:
            xsradLong3[0].append(Ebeam - d[1]/1000)
            xsradLong3[1].append(d[2])
            xsbornLong3[0].append(Ebeam - d[1]/1000)
            xsbornLong3[1].append(d[3])
        for d in dataTrans3:
            xsradTrans3[0].append(Ebeam - d[1]/1000)
            xsradTrans3[1].append(d[2])
            xsbornTrans3[0].append(Ebeam - d[1]/1000)
            xsbornTrans3[1].append(d[3])
        ...
        for i in range(len(dataLong3)):
            if (dataLong2[i][3] == 0): continue
            tempEp = Ebeam - (dataLong3[i][1] + dataUnpol3[i][1])/2000
            tempx = x_calc(tempEp, theta)
            if (tempx > 1): continue
            Apar3_rad[0].append(tempx)
            Apar3_born[0].append(tempx)
            Apar3_rad[1].append(dataLong3[i][2]/(2.0*dataUnpol3[i][2]))
            Apar3_born[1].append(dataLong3[i][3]/(2.0*dataUnpol3[i][3]))
            Apar3_diff[0].append(tempx)
            Apar3_diff[1].append(dataLong3[i][3]/(2.0*dataUnpol3[i][3]) - dataLong3[i][2]/(2.0*dataUnpol3[i][2]))
            
        for i in range(len(dataTrans3)):
            if (dataTrans3[i][3] == 0): continue
            tempEp = Ebeam - (dataTrans3[i][1] + dataUnpol3[i][1])/2000
            tempx = x_calc(tempEp, theta)
            if (tempx > 1): continue
            Aperp3_rad[0].append(tempx)
            Aperp3_born[0].append(tempx)
            Aperp3_rad[1].append(dataTrans3[i][2]/(2.0*dataUnpol3[i][2]))
            Aperp3_born[1].append(dataTrans3[i][3]/(2.0*dataUnpol3[i][3]))
            Aperp3_diff[0].append(tempx)
            Aperp3_diff[1].append(dataTrans3[i][3]/(2.0*dataUnpol3[i][3]) - dataTrans3[i][2]/(2.0*dataUnpol3[i][2]))
        ...
        plt.plot(xsradUnpol2[0], xsradUnpol2[1], color="blue", linestyle='--', label='XS rad ' + gmLabel(theta))
        plt.plot(xsbornUnpol2[0], xsbornUnpol2[1], color="green", linestyle='--', label='XS born ' + gmLabel(theta))
        plt.plot(xsradUnpol3[0], xsradUnpol3[1], color="blue", linestyle=':', label='XS rad ' + gpLabel(theta))
        plt.plot(xsbornUnpol3[0], xsbornUnpol3[1], color="green", linestyle=':', label='XS born ' + gpLabel(theta))        
        plt.plot(horizLineUnpol[0], horizLineUnpol[1], color='black', linestyle='--')
        plt.xlabel('E\' [GeV]')
        plt.ylabel('XS')
        plt.title(unpolLabel2 + ' E = ' + energyStr + ' MeV 'r'$\theta$ = ' + thetaStr + degree_sign + ' Inelastic + Quasi-Elastic Smearing with Perturbations to 'r'$\mathrm{g}_1$')
        plt.grid(False)
        plt.legend()
        # plt.show()
        plt.tight_layout()
        ...
        plt.plot(xsradLong2[0], xsradLong2[1], color="blue", linestyle='--', label='XS rad ' + gmLabel(theta))
        plt.plot(xsbornLong2[0], xsbornLong2[1], color="green", linestyle='--', label='XS born ' + gmLabel(theta))
        plt.plot(xsradLong3[0], xsradLong3[1], color="blue", linestyle=':', label='XS rad ' + gpLabel(theta))
        plt.plot(xsbornLong3[0], xsbornLong3[1], color="green", linestyle=':', label='XS born ' + gpLabel(theta))
        plt.plot(horizLineLong[0], horizLineLong[1], color='black', linestyle='--')
        plt.xlabel('E\' [GeV]')
        plt.ylabel('XS')
        plt.title(longLabel2 + ' E = ' + energyStr + ' MeV 'r'$\theta$ = ' + thetaStr + degree_sign + ' Inelastic + Quasi-Elastic Smearing  with Perturbations to 'r'$\mathrm{g}_1$')
        plt.grid(False)
        plt.legend()
        # plt.show()
        plt.tight_layout()
        ...
        plt.plot(xsradTrans2[0], xsradTrans2[1], color="blue", linestyle='--', label='XS rad ' + gmLabel(theta))
        plt.plot(xsbornTrans2[0], xsbornTrans2[1], color="green", linestyle='--', label='XS born ' + gmLabel(theta))
        plt.plot(xsradTrans3[0], xsradTrans3[1], color="blue", linestyle=':', label='XS rad ' + gpLabel(theta))
        plt.plot(xsbornTrans3[0], xsbornTrans3[1], color="green", linestyle=':', label='XS born ' + gpLabel(theta))
        plt.plot(horizLineTrans[0], horizLineTrans[1], color='black', linestyle='--')
        plt.xlabel('E\' [GeV]')
        plt.ylabel('XS')
        plt.title(transLabel2 + ' E = ' + energyStr + ' MeV 'r'$\theta$ = ' + thetaStr + degree_sign + ' Inelastic + Quasi-Elastic Smearing with Perturbations to 'r'$\mathrm{g}_1$')
        plt.grid(False)
        plt.legend()
        # plt.show()
        plt.tight_layout()
        ...
        plt.plot(Apar2_rad[0], Apar2_rad[1], color="blue", linestyle='--', label=AparLabel2+' rad ' + gmLabel(theta))
        plt.plot(Apar2_born[0], Apar2_born[1], color="green", linestyle='--', label=AparLabel2+' born ' + gmLabel(theta))
        plt.plot(Apar3_rad[0], Apar3_rad[1], color="blue", linestyle=':', label=AparLabel2+' rad ' + gpLabel(theta))
        plt.plot(Apar3_born[0], Apar3_born[1], color="green", linestyle=':', label=AparLabel2+' born ' + gpLabel(theta))
        ...
        plt.title(AparLabel2 + ' E = ' + energyStr + ' MeV 'r'$\theta$ = ' + thetaStr + degree_sign + ' Inelastic + Quasi-Elastic Smearing with Perturbations to 'r'$\mathrm{g}_1$')
        plt.grid(False)
        plt.legend()
        # plt.show()
        plt.tight_layout()
        ...
        plt.plot(Aperp2_rad[0], Aperp2_rad[1], color="blue", linestyle='--', label=AperpLabel2+' rad ' + gmLabel(theta))
        plt.plot(Aperp2_born[0], Aperp2_born[1], color="green", linestyle='--', label=AperpLabel2+' born ' + gmLabel(theta))
        plt.plot(Aperp3_rad[0], Aperp3_rad[1], color="blue", linestyle=':', label=AperpLabel2+' rad ' + gpLabel(theta))
        plt.plot(Aperp3_born[0], Aperp3_born[1], color="green", linestyle=':', label=AperpLabel2+' born ' + gpLabel(theta))
        ...
        plt.title(AperpLabel2 + ' E = ' + energyStr + ' MeV 'r'$\theta$ = ' + thetaStr + degree_sign + ' Inelastic + Quasi-Elastic Smearing with Perturbations to 'r'$\mathrm{g}_1$')
        plt.grid(False)
        plt.legend()
        # plt.show()
        plt.tight_layout()
        ...
        plt.plot(Apar2_diff[0], Apar2_diff[1], color="blue", linestyle='--', label=AparLabel2+'(born - rad) ' + gmLabel(theta))
        plt.plot(Apar3_diff[0], Apar3_diff[1], color="blue", linestyle=':', label=AparLabel2+'(born - rad) ' + gpLabel(theta))
        plt.plot(horizLineAperp[0], horizLineAperp[1], color='black', linestyle='--')
        plt.xlabel('x')
        plt.ylabel(AparLabel2 + '(born - rand)')
        plt.title(AparLabel2 + '(born - rad) E = ' + energyStr + ' MeV 'r'$\theta$ = ' + thetaStr + degree_sign + ' Inelastic + Quasi-Elastic Smearing with Perturbations to 'r'$\mathrm{g}_1$')
        plt.grid(False)
        plt.legend()
        # plt.show()
        plt.tight_layout()
        ...
        plt.plot(Aperp2_diff[0], Aperp2_diff[1], color="blue", linestyle='--', label=AperpLabel2+'(born - rad) ' + gmLabel(theta))
        plt.plot(Aperp3_diff[0], Aperp3_diff[1], color="blue", linestyle=':', label=AperpLabel2+'(born - rad) ' + gpLabel(theta))
        plt.plot(horizLineAperp[0], horizLineAperp[1], color='black', linestyle='--')
        plt.xlabel('x')
        plt.ylabel(AperpLabel2 + '(born - rand)')
        plt.title(AperpLabel2 + '(born - rad) E = ' + energyStr + ' MeV 'r'$\theta$ = ' + thetaStr + degree_sign + ' Inelastic + Quasi-Elastic Smearing with Perturbations to 'r'$\mathrm{g}_1$')
        plt.grid(False)
        plt.legend()
        # plt.show()
        plt.tight_layout()
